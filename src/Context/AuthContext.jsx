...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (id, email, password) => {
    try {
      ...
      await axios.post(`http://localhost:8080/api/ipadress/registerIp/${user_id}`, {
        ip: ipInfo.ip,
      });
  ...
  const confirmEmail = async (token) => {
    try {
      const response = await axios.post(`http://localhost:8080/api/user/confirm-email/${token}`);
  ...
  const login = async (userid, password) => {
    try {
      ...
      try{
        await axios.post(
          `http://localhost:8080/api/ipadress/verifyIp/${response.data.id}`,
          {
            ip: ipInfo.ip,
          }
        );
        ...
        if (response.data.status === "Pending") {
          Swal.mixin({
            ...
            .then(async () => {
              await axios.post(
                `http://localhost:8080/api/user/resend-email/${response.data.token}`
              );
            });
          return;
  ...
  const completeRegister = async ( first_name, last_name, date_of_birth, address, phone_number ) => {
    try {
      ...
      await axios.post(`http://localhost:8080/api/user/complete-register/${id}`, {
  ...
  const restorePassword = async (password) => {
    try {
      ...
      const response = await axios.post(`http://localhost:8080/api/user/restore-password/${decodedToken}`, {
        password: sanitizedPassword,
      });
      console.log(response.data);
      Swal.mixin({
