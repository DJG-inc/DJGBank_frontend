...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (id, email, password) => {
    try {
      ...
      const response = await axios.post("https://djgbank-backend.fly.dev/api/user/register", {
        user_id: sanitizedId,
      ...
      await axios.post(`https://djgbank-backend.fly.dev/api/ipadress/registerIp/${user_id}`, {
        ip: ipInfo.ip,
      });
  ...
  const confirmEmail = async (token) => {
    try {
      const response = await axios.post(`https://djgbank-backend.fly.dev/api/user/confirm-email/${token}`);
  ...
  const login = async (userid, password) => {
    try {
      ...
      const response = await axios.post(
        "https://djgbank-backend.fly.dev/api/user/login",
        {
      ...
      try{
        await axios.post(
          `https://djgbank-backend.fly.dev/api/ipadress/verifyIp/${response.data.id}`,
          {
            ip: ipInfo.ip,
          }
        );
        ...
        if (response.data.status === "Pending") {
          Swal.mixin({
            ...
            .then(async () => {
              await axios.post(
                `https://djgbank-backend.fly.dev/api/user/resend-email/${response.data.token}`
              );
            });
          return;
  ...
  const completeRegister = async ( first_name, last_name, date_of_birth, address, phone_number ) => {
    try {
      ...
      await axios.post(`https://djgbank-backend.fly.dev/api/user/complete-register/${id}`, {
  ...
  const forgotPassword = async (email) => {
    try {
      const sanitizedEmail = DOMPurify.sanitize(email);
      const response = await axios.post("https://djgbank-backend.fly.dev/api/user/forgot-password", {
        email: sanitizedEmail,
      });
  ...
  const restorePassword = async (password) => {
    try {
      ...
      const response = await axios.post(`https://djgbank-backend.fly.dev/api/user/restore-password/${decodedToken}`, {
        password: sanitizedPassword,
      });
      console.log(response.data);
      Swal.mixin({
